export interface OneTabUrlQueryCleanupHelpers {
  cleanUrlForSearch(url: string): string;
}

export function createOneTabUrlQueryCleanup(): OneTabUrlQueryCleanupHelpers {
  function cleanUrlForSearch(url: string) {
    ...
    hashIndex !== -1 &&
      ((hash = base.slice(hashIndex)), (base = base.slice(0, hashIndex)));
    ...
    for (const piece of pieces) {
      ...
      if (isSohuTrackingParam(parsed, key) || alwaysNoisyParams.has(key))
        continue;
      (likelyUsefulParams.has(key) ||
        !looksLikeTrackingToken(value, key, noisyWhenEncoded)) &&
        kept.push(piece);
    }
  ...
  function isSohuTrackingParam(url: URL | null, key: string) {
    ...
    return !!(
      key === "spm" &&
      (hostname === "sohu.com" || hostname.endsWith(".sohu.com"))
    );
  }

  function looksLikeTrackingToken(
    value: string,
    key: string,
    noisyKeys: Set<string>,
  ) {
    if (value.length <= 12 || /[^\x00-\x7F]/.test(value) || /^\d+$/.test(value))
      return !1;
    const uuid =
    ...
    if (
      ...
      base64Url ||
      jwt ||
      (/[A-Za-z0-9+/_\-]{24,}={0,2}/.test(value) && noisyKeys.has(key))
    )
      return !0;
    if (
      ...
      for (let index = 1; index < value.length; index++)
        (/[A-Z]/.test(value[index - 1]) &&
          /[a-z]/.test(value[index]) &&
          caseTransitions++,
          /[a-z]/.test(value[index - 1]) &&
            /[A-Z]/.test(value[index]) &&
            caseTransitions++);
      const parts = value.split(/[-_.]/).filter((part) => part.length > 0),
        manyShortParts =
          parts.length >= 6 && parts.every((part) => part.length <= 12),
    ...
    return (
      /\s/.test(value) ||
        (/[-_.]/.test(value) &&
          value.split(/[-_.]/).some((part) => /[A-Za-z]{3,}/.test(part))),
      !1
    );
  }
