export interface OneTabImportHelperDependencies {
...
export interface OneTabImportedTabItem {
  kt: string | undefined;
  title?: string;
  url?: string;
  Zo?: string[];
...
export interface OneTabImportHelpers {
...
export function createOneTabImportHelpers({
  combineComparators,
  compareAscendingBy,
  compareDescendingBy,
...
`,
      ...
      let urlOnly = /^(?![A-Za-z]:[\\/])[a-zA-Z][a-zA-Z0-9+.-]*:(?:\/\/)?\S+$/,
        items: any[] = [];
