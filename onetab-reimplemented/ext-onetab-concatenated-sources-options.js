...
const ja = (i) => i === void 0,
  ...
  V = (i) => i.id === "root",
  yt = (i) => i.id === "trash",
  P = (i) => i.groupType === "folder",
  Zt = (i) => i.archived,
  He = (i) => i.task,
  Na = (i) => P(i) && !yt(i),
  hi = (i) => i.groupType === "quickList",
  Ca = (i) => P(i) || vt(i),
  _ = (i) => i.type === "group",
  ct = (i) => i.groupType === "tabGroup",
  vt = (i) => i.groupType === "window",
  Jt = (i) => ct(i) || vt(i),
  ot = (i) => i !== "quickList",
  Qn = (i) => i.shared && i.shareExpiryDate > +new Date();
function Ra(i, t) {
...
function _n({ text: i, ti: t, ei: e, ii: s }) {
  let n = [{ s: i, tt: 0 }];
  return (
    t.sort(_t((r) => r.length)).forEach((r) => {
      n = n.flatMap(({ s: o, tt: a }) =>
        a ? { s: o, tt: a } : Yn(o, r, e, s),
      );
    }),
    n
  );
...
function Yn(i, t, e, s) {
  let n = i.split(ds(t, s, e)),
    r = 0,
    o = n.map((a) => {
      let l = i.slice(r, r + a.length);
      return ((r += a.length + t.length), { s: l, tt: 0 });
    });
  return (
    (r = 0),
    o.length < 2
      ? o
      : o.flatMap(({ s: a, tt: l }, u) =>
          u === o.length - 1
            ? { s: a, tt: l }
            : ((r += a.length),
              [
                { s: a, tt: l },
                { s: i.slice(r, (r += t.length)), tt: 1 },
              ]),
        )
  );
}
...
function di(i, t, e) {
  return Array.isArray(i)
    ...
    : typeof i == "object" && i !== null
      ? Object.fromEntries(Object.entries(i).map(([s, n]) => [s, di(n, t, e)]))
      : i === t
        ? e
        : i;
}
...
function xs(i) {
  ...
  let e = t.includes("://") ? t : `https://${t}`,
    s = At(e);
  return (s.endsWith("/") && (s = s.substring(0, s.length - 1)), qt(s));
}
function ws(i, t) {
  let e = Yt(i),
    s = Yt(t);
  return !e || !s ? !1 : qe(e, s) ? !0 : xs(e) === xs(s);
}
function fr(i) {
  try {
    let t = new URL(At(i)).hostname.toLowerCase();
    return (
      t === "youtube.com" ||
...
function dr(i, t, e) {
  ...
  return !n || ws(n, t)
    ? !0
    : fr(t) && n.toLowerCase() === "youtube"
      ? s.toLowerCase() !== "youtube" && !ws(s, t)
      : !1;
}
function re(i) {
  let t = At(i);
...
async function ms(i) {
    ...
    n;
  {
    ...
    [...e.querySelectorAll("a")].forEach((a) => {
      let l = a.href,
        u = re(a.href),
        d = zt(s.get(l) ?? a.textContent);
      (qe(d, u) && (d = qt(d)), u && d && r.push({ url: l, kt: u, title: d }));
    });
    ...
    n = Object.values(o).map(
      (a) =>
        a.sort(
          ne(
            fs((l) => qe(l.title, l.url)),
            _t((l) => l.title.length),
          ),
        )[0],
    );
  }
  if (n.some((r) => !qe(r.url, r.title))) return [n];
  {
    let r = [
      "address",
      "article",
      ...
      [...x.childNodes].reduce(
        (m, g) =>
          g.nodeType === 3
            ? `${m}${g.textContent}`
            : `${m}${
                r.some((w) => w.toUpperCase() === g.tagName?.toUpperCase())
                  ? `
${o(g)}`
                  : o(g)
              }`,
...
`,
          ...
          .map((m) => zt(m))
          .forEach((m) => {
            if (!m.trim()) x[x.length - 1].length && x.push([]);
            else {
              let g = m.match(l);
              if (g) {
                let w = re(g.groups.url);
                if (w) {
                  let T = g.groups.title || s.get(g.groups.url) || qt(w),
                    f = { kt: w, title: T };
                  x[x.length - 1].push(f);
                }
              }
    ...
    for (let x = 0; x < u.length; x++) {
      ...
      if (d.test(m)) {
        ...
        T && !d.test(T)
          ? ((w.title = T), g?.Zo.pop())
          : (w.title = s.get(m) || qt(w.kt));
      } else g?.Zo.push(m);
    }
    p.forEach((x) => {
      ((x.Ja = x.Zo.filter((m) => m).join(`
`)),
        x.Ja || delete x.Ja);
    });
    let b = /(?<url>(?![A-Za-z]:[\\/])[a-zA-Z][a-zA-Z0-9+.-]*:(?:\/\/)?\S+)/g,
      y = u
        .flatMap((x) => [...x.matchAll(b)].map((m) => re(ur(m.groups.url))))
        .filter((x) => x);
    return (
      (y = [...new Set(y)]),
      y.length > p.length ? [y.map((x) => ({ kt: x, title: qt(x) }))] : [p]
    );
  }
}
function mt(i) {
...
function Rr({ fontSize: i, label: t, isSelected: e, St: s }) {
  return c({
    className: ["multiple-choice-button", ...$(e, "on")].join(" "),
    style: { fontSize: i },
    textContent: t,
    onclick: (n) => s(n),
  });
      ...
      children: {
        heading: c({
          ...
          children: {
            ...
            text: c({ textContent: t }),
          },
        }),
        ...(e && { Dd: c({ style: { fontSize: "12px" }, textContent: e }) }),
...
class St extends Ii {
  constructor({ To: t, heading: e, options: s, note: n, icon: r, style: o }) {
    (super({ heading: e, icon: r, note: n, style: o }),
      (this.To = t),
      (this.options = s),
      (this.mn = this.options.map((a) => new Wr({ aa: this, mi: a }))),
      this.Pd(
        c({
          style: {
            display: "flex",
  ...
  async Lt() {
    let t = await v.ks(this.To);
    await this.wn(t);
  }
  async wn(t, e) {
    (this.mn.forEach((s) => s.wn(t)), e && (await v.Ve(this.To, t)));
  }
}
class Wr {
  wn(t) {
    ...
    this.o = c({
      ...
      children: {
        ...
        ow: c({
          ...
          children: {
            label: c({ style: { fontSize: "14px" }, textContent: e.title }),
            ...(e.xi && {
              xi: c({
                style: {
                  color: "var(--text-color-weak)",
            ...
            ...(e.Fc && { Fc: c({ style: { marginTop: "8px" }, p: e.Fc }) }),
          },
        }),
      },
    });
...
class dt {
  constructor({
    ...
    ((this.ua = []),
      (this.o = c({
        ...
        style: {
          fontSize: 0,
          position: "absolute",
          zIndex: $t.Is ? $t.zIndex + 1 : 1e3,
          borderRadius: s,
        },
        p:
          t ??
          Pi({ style: { marginInline: "4px 14px", marginBlock: "10px 10px" } }),
      })),
      (this.m ??= []).push(J(this.o.i, "click", (d) => d.stopPropagation())));
    ...
    ((this.o.i.style.left = ""),
      ...
      (this.m ??= []).push(J(n, "click", (d) => this.destroy())),
      (this.m ??= []).push(J(n, "contextmenu", (d) => this.destroy(), !0)),
      (dt.rn = () => this.destroy()),
      o &&
        this.m.push(
          J(document, "keydown", (d) => {
            he(d) && this.destroy();
          }),
        ),
      r?.(this));
...
class $l {
  constructor({ onChange: t, lw: e, options: s }) {
    ((this.onChange = t),
      (this.value = e),
      (this.mn = s.map(
        ({ label: n, value: r }) => new zr({ aa: this, label: n, value: r }),
      )),
      (this.o = c({
  ...
  le(t) {
    if (t !== this.isVisible) {
      if (t) {
        ...
        (this.onShown?.(this), (dt.rn = () => this.le(!1)));
      ...
      ((this.isVisible = t),
        ...
        t
          ? ((this.o.i.style.maxWidth = `${this.zs}px`),
            ...
            (this.fa = J(document, "keydown", (e) => {
              he(e) && (this.da(), document.activeElement?.blur());
            })),
            (this.m ??= []).push(this.fa),
            ut?.destroy(),
...
function Qr({ style: i }) {
  return c({
    ...
    children: {
      Bc: c({
        children: {
          info: M("span", { textContent: h("pasteRichOrPlainTextBelow") }),
          Bd: M("span", {
            textContent: h("seePasteInstructions"),
            style: {
              marginInlineStart: "1ex",
              color: "var(--link-color)",
              cursor: "pointer",
      ...
      Cd: c({
        style: { display: "none" },
        children: [
          c({ textContent: h("canPasteWebContentForExample") }),
          (() => {
            let t = (o, a) =>
                M("a", {
                  textContent: o,
              ...
              e = (o) => (o === "" ? void 0 : M("span", { textContent: o })),
              [s, n, r] = h("exampleRichTextParagraph")
                .split("PLACEHOLDER")
                .map(e);
            return c({
              ...
              children: [
                s,
                t(h("exampleLink"), "https://www.example.com/one"),
                n,
                t(h("secondExampleLink"), "https://www.example.com/two"),
                r,
              ].filter((o) => o),
            });
          })(),
          c({
            ...
            textContent: h("plainTextLineFormat"),
          }),
          c({
            className: "linkImportExample",
            children: [
              c({
                textContent: `https://www.example.com/one | ${h("exampleLinkTitle")}`,
              }),
              c({
                textContent: `https://www.example.com/two | ${h("secondExampleLinkTitle")}`,
              }),
            ],
          }),
          c({
            style: { paddingTop: "8px" },
            textContent: h("alternatePlainTextFormat"),
          }),
          c({
            style: { "unicode-bidi": "plaintext" },
            className: "linkImportExample",
            children: [
              c({ textContent: h("exampleLinkTitleOptional") }),
              c({ textContent: "https://www.example.com/one" }),
              c({ textContent: h("notesAboutTheLinkOptional") }),
              c({ style: { paddingTop: "14px" } }),
              c({ textContent: h("secondExampleLinkTitleOptional") }),
              c({ textContent: "https://www.example.com/two" }),
              c({ textContent: h("notesAboutTheSecondLinkOptional") }),
            ],
          }),
          c({
            style: { paddingTop: "10px", paddingBottom: "14px" },
            textContent: h("canDragTextNote"),
          }),
        ],
      }),
    },
...
class Kr {
  constructor({ Cc: t, cw: e, xn: s, pa: n, ma: r, wa: o }) {
    ...
    ((this.m ??= []).push(J(this.o.be.Ze.i, "keydown", a, !0)),
      (this.m ??= []).push(
        J(this.o.be.Ze.i, "input", (l) => {
      ...
      if (this.ti?.length) {
        ...
        if (t.length) {
          ...
          this.Ct.o.vs.body.i.replaceChildren(
            c({ style: {}, textContent: h("noMatchesFound") }).i,
          );
        this.Ct.xa(!0);
      } else this.Ct.xa(!1);
  }
...
class Vr {
  constructor({
    ...
    this.o = c({
      className: ["selectable-item", ...$(e, "on")].join(" "),
      style: {
        display: "inline-block",
        textIndent: 0,
        ...(r && { margin: r }),
...
class Y {
  ...
  async st({ ta: t, ys: e, pinned: s, xe: n, wt: r, kn: o, h: a, ut: l }) {
    ((this.l = { ...t }),
      ...
      (this.kn = o));
    let u = Fn(At(this.l.url), this.xo);
    if ((this.Qc(), this.J)) {
      this.nn = structuredClone(this.l);
      let y = await cs(this.X),
        x = Object.entries(y);
      x.length > 0 &&
        (x.forEach(([m, g]) => (this.l[m] = g)),
        (this.Rt = !0),
        this.It({ de: !0, pe: !1, X: this.X, Ui: this.yo }));
    }
    let d, p;
    if (this.gn) {
      let { o: y, Nd: x } = await this.Jc({ h: a });
      ((d = y), (p = x));
    }
    let b = this.o.i;
    ((this.o = c({
        ...
        ((this.m ??= []).push(
          Oe(y.i, (x) => {
            !F.Xr &&
              ...
              this.K &&
                this.it &&
                this.o.tab.i.insertBefore(Mi(), this.o.tab.i.children.item(0)));
          }),
        ),
          (this.m ??= []).push(
            Me(y.i, (x) => {
        ...
        tab: c({
          ...
          children: {
            zi: c({
              ...
              p: Ri(this.l.url, this.Ft && !this.ut ? "move" : "default"),
            }),
            ba: M("a", {
              draggable: !1,
              ...(!this.kn && { tabIndex: -1 }),
              ...
              children: {
                So: M("span", {
                  ...
                  textContent: oi(this.l.title, "Untitled"),
                }),
              },
              href: u,
              onclick: (y) =>
          ...
          Wi: M("div", {
            ...
            children: {
              a: M("a", {
                ...
                children: {
                  dw: M("span", {
                    textContent: Ue(prettyPrintUrl(this.l.url), (y) =>
                      Ys(this.Mt === "domain" ? ii(y) : _i(y), _s),
                    ),
                  }),
                },
              }),
  ...
  async Jc({ h: t }) {
    let e = "11px;",
      s;
    this.C && (s = h("otherCopiesColon"));
    let n = this.l.id;
    await t.ke();
    let r = await lt(n),
      o = await Promise.all(
        r.parentIds
          .filter((l) => ot(l) && !t.re.has(l))
          .map((l) => {
            let u = t.v(l);
            return {
              parentId: l,
    ...
    if (!o.length) a = c({});
      ...
      let l = () =>
        M("span", { style: { textAlign: "start" }, textContent: ",   " });
      a = c({
        style: { textAlign: "start", marginInlineEnd: "20px" },
        children: {
          Yd: (() => {
            let [u, d] = Ft("placeholderAndOtherLocations", o.length - 1).split(
                "PLACEHOLDER",
              ),
              p = [u, o[0], d].filter((b) => b);
            return (
              (p = p.map((b) =>
                typeof b == "string"
                  ? M("span", {
  ...
  us(t, e, s) {
    (mi({ label: zt(this.l.title), Ro: this.o.u.So.i, oe: t, ei: e, ii: s }),
      this.Y?.us(t, e, s),
      this.o.u.Wi &&
        mi({
  ...
  async Ge(t, e) {
    ...
    if (t.type === "update" && this.l.id === s) {
      if (
        (ht(t, "title", (n) => this.setTitle(n)),
        ht(t, "notes", (n) => this.wo(n)),
        ht(t, "rating", (n) => this.fi(n)),
        ht(t, "archived", (n) => this.Bi(n)),
        be.some((n) => t.propChanges[n]))
      ) {
      ...
      (t.type === "copy" && jt(this.l.id, t.itemIds))
    ) {
      let n = await lt(this.l.id);
  ...
  setTitle(t) {
    ((this.l.title = t),
      (this.o.u.So.i.textContent = oi(t, h("untitled"))),
      this.Je?.et(t));
  }
  wo(t) {
    ...
    (this.Yc(),
      this.Je ||
        ((this.Je = new oe({
          ...
          ue: this.ue,
          value: zt(this.l.title),
          rs: "tabLinkText",
          Gi: h("title"),
          Qs: (t) => (this.o.u.So.i.textContent = oi(t, h("untitled"))),
        })),
      ...
      !this.ie &&
        ...
        ((this.ie = new oe({
          style: {
            display: "none",
            marginInlineStart: `${K() ? 49 : 10}px`,
            marginInlineEnd: `${K() ? 10 : 49}px`,
            marginBottom: "5px",
            direction: "ltr",
      ...
      ((this.it = W(
        {
          ...
          init: (t) => {
            ((this.m ??= []).push(
              J(t.i, "click", (e) => this.lt(!this.Gt(), !1, Ze(e))),
            ),
              (this.m ??= []).push(J(t.i, "contextmenu", (e) => this.ya(e))));
          },
        },
        ...
        ns: async () => {
          ...
          if (this.hn)
            if (!this.ie.value) this.ie.et(this.l.url);
              ...
              let r = At(this.ie.value);
              if (r !== this.l.url) {
                ((n = !1), (this.tu = !0));
                let [o] = await v.Xo([
                    { id: await ni(r), kt: r, oh: [this.xe] },
                  ]),
                  a = o;
                (Object.entries(this.l)
                  .filter(([p]) => !["id", "url", "parentIds"].includes(p))
                  .forEach(([p, b]) => (a[p] = b)),
                  (a.parentIds = [this.xe]));
                let l = this.wt.k
                  .map((p, b) => [p, b])
                  .filter(([p]) => p.l.id === this.l.id)
                  .map(([p, b]) => b);
                await v.In({ vn: [a], O: this.xe, N: this.index, An: [a.id] });
                let u = new kt();
                if (
                  (await u.ke(),
                  !(
                    this.l.parentIds.filter(
                      (p) => ot(p) && !u.re.has(p) && p !== this.xe,
                    ).length || l.length > 1
                  ) && this.l.parentIds.includes("quickList"))
                ) {
                  let b = (await lt("quickList")).childIds.indexOf(this.l.id);
                  await v.eu({ iu: [a.id], O: "quickList", N: b });
                }
                await v.move({ rt: [Te(this)], O: "delete" });
              }
            }
          if (n) {
            let r = {
  ...
  Jd(t) {
    ...
    if (
      ...
      new Set(s.map((l) => Fi(l.l))).size > 1)
    )
      return ($t.Is || te(h("cannotMixTabsGroupsFoldersDuringDrag")), !1);
    dnd.ft.types = ["tab", ...$(this.C, "trash"), ...$(this.ut, "locked")];
    let r = [this.o.i];
    e.map((l) => l.o.i).forEach((l) => {
      r.includes(l) || r.push(l);
    });
    let o = e.map(Te);
    (e.includes(this) || o.push(Te(this)),
      (dnd.ft.Ae = {
        itemsMeta: o,
...
function Mi() {
  return (
    (Bs ??= c({
      ...
      style: {
        [K() ? "left" : "right"]: "-31px",
        boxShadow: `var(--tick-cross-box-shadow${K() ? "" : "-rtl"})`,
      },
    }).i),
    Bs
  );
    ...
    ((this.We = s),
      ...
      (this.o = c({
        ...
        children: {
          Qr: new Mt({
            ni: !0,
            label: h(a ? "deleteThisCopy" : "delete"),
            icon: { I: "cross-action", dimension: "10px" },
            onclick: this.Ne,
          }),
          Jr: new Mt({
...
class Zi {
  ...
  async st({ h: t, ka: e, sf: s } = {}) {
    t ??= (await new kt().Te({ groupId: this.itemId })).h;
    let n = t.v(this.itemId);
    this.l = { ...n };
    let r = this.l.id === "trash" ? [] : [this.l.id];
    e ??= await v.pw(`treeItemsOpen:${this.q}`, r);
    let o = e.includes(this.l.id),
      a,
      l;
    if (this.l.type === "tab") a = this.l.title;
    else {
      let [m, g] = Tr({ h: t, l: this.l });
      ((a = m), (l = g));
    }
    let u = j(this.l) ? 1 : t.Xi(this.l.id),
      d = this.Ht && t.ll(this.l.id).cl,
      p = this.Ht ? d : u,
      b = Number(p).toLocaleString();
    this.Ht && !p && (b = "");
    let y = "none";
    (P(this.l) && (y = "folder"),
      yt(this.l) && (y = "trash"),
    ...
    (x && this.De({ pi: !0 }),
      (this.o = c({
        ...
        children: {
          mw: new to({ ot: this.ot, tf: this.au }),
          se: c({
                ...
                children: {
                  ...(this.A &&
                    !this.ot && {
                      Pe: new Fr({
                        ...
                        onChange: (m, g) => {
                          (this.A.Ao({ newState: m, selection: this, Nc: g }),
                      ...
                      p: W(
                        {
                          ...
                          l: { onclick: (m) => this.setActive(!0, !0) },
                        },
                        (m) => `images/folder${m ? "" : "-dark"}.png`,
                      ),
                    }),
                  }),
                  ...(this.icon === "trash" && {
                  ...
                  Ut: new oe({
                    ...
                    Qs: async (m) => {
                      ...
                      let g = j(this.l) ? "title" : "label",
                        w = this.l[g];
                      if (((this.l[g] = m), w !== this.l[g])) {
                        let T = { [g]: this.l[g] };
                        await v.Fi(this.itemId, T);
                      }
                    },
                  }),
                  uu: c({
      ...
      (this.m ??= []).push(
        ...dnd.ir({
          ...
          ar: (m) => {
            ...
            let T = g.map(Te);
            g.includes(this) || T.push(Te(this));
            let f = [...g];
            f.includes(this) || f.push(this);
            let k = eo(f);
            if (new Set(f.map((S) => Fi(S.l))).size > 1)
              return (
                $t.Is || te(h("cannotMixTabsGroupsFoldersDuringDrag")),
                !1
              );
            ((dnd.ft.types = k),
              (dnd.ft.Ae = {
            ...
            let L = j(this.l)
              ...
              : P(this.l)
                ...
                : "dragged-group";
            (f.length > 1 &&
              (f.every((S) => _(S.l) && !P(S.l))
                ? (L = "multi-dragged-group")
                : f.every((S) => P(S.l))
                  ? (L = "multi-dragged-folder")
                  : f.every((S) => j(S.l))
                    ? (L = "multi-dragged-tab")
                    : (L = "multi-dragged-item")),
              Ei({ e: m, Kr: L, ji: w, q: this.q }));
          },
          lr: () => {
            (Xe(),
              this.qt &&
        ...
        ...dnd.hr({
          ...
          dr: (m) => {
            let g = this.o.u.body.Ut.i,
              T = this.o.u.se.i.getBoundingClientRect(),
              f = g.getBoundingClientRect(),
              k = Math.floor(f.height * 0.2);
            ...
            let at = !(
                ...
                j(this.l) ||
                H.some((et) => S.includes(et.itemId))
              ),
              q = !(
                H.some((et) => L.includes(et.itemId)) ||
                (Q.includes("tab") && this.q !== "quickAccessCol-quickList") ||
                (this.B && Xt(["window", "tabGroup"], Q)) ||
                (this.Ot &&
                  Xt(["window", "tabGroup"], Q) &&
                  Q.includes("untitled")) ||
                (this.L.l.id === "trash" && this.l.id !== "trash")
              );
            if (!at && !q) return !1;
            let z = at ? "within" : "before";
            !this.ot &&
              q &&
              (m.clientY <= f.y + Math.max(9, k) && (z = "before"),
              this.isOpen
                ? m.clientY > T.y + T.height - 2 && (z = "after")
                : at
                  ? m.clientY > f.y + f.height - Math.max(9, k) && (z = "after")
                  : m.clientY > f.y + f.height / 2 && (z = "after"));
            let Z =
              this.ot &&
            ...
            if (Z) {
              let et = this.Ke?.i,
                Pt = this.S.map((X) => X?.o?.i)
                  .filter((X) => X?.parentElement === et)
                  .at(-1);
              if (Pt) {
                let X = Pt.getBoundingClientRect();
                m.clientY > X.bottom - 2 && (z = "after");
              }
            }
            let Tt = Z && z === "after";
            dnd.P.Le = z;
            let U = z === "within" || Tt ? this : this.G;
            return !U ||
              !Ui({
                Er: Q,
                Fr: U.l.groupType,
              ...
              vi(m)
              ? !1
              : U.itemId === "quickList" && !dnd.F
                ? (delete Qt.Le, xt.remove(), !1)
                : U.q === "quickAccessCol-tasks" && U.itemId === "root"
                  ? (delete Qt.Le, xt.remove(), !1)
                  : !0;
          },
          pr: (m) => {
                ...
                : g === "after"
                  ...
                  : (T.appendChild(xt),
                    Object.assign(xt.style, {
                      ...
                      left: "-3px",
                      right: "",
                      width: `${T.clientWidth + 3}px`,
                      borderWidth: "3px",
                    })),
              (Qt.Le = g));
          },
          mr: () => {
            (delete Qt.Le, xt.remove());
          },
          wr: async (m) => {
            if (this.j) return;
            let g = dnd.P.Le ?? Qt.Le;
            if ((delete Qt.Le, xt.remove(), !g)) return;
            let w =
              this.ot &&
              this.q === "quickAccessCol-quickList" &&
              this.itemId === "quickList" &&
            ...
            (g !== "within" && !w && !this.G?.Ke?.i.contains(this.o.i)) ||
              ...
              await Bi({
                ...
                Hr: () => {
                  ...
                  if (!f) return;
                  let k = [...f.children],
                    A = w ? k.length : k.findIndex((z) => z === this.o.i);
                  if (A < 0) {
                    let z = T.S.filter((Z) => Z?.o?.i?.parentElement === f);
                    return z.length ? z.at(-1)?.index + 1 : void 0;
                  }
                  g === "after" && !w && A++;
                  let L = new Map();
                  T.S.forEach((z) => {
                    let Z = z?.o?.i;
                    Z && Z.parentElement === f && !L.has(Z) && L.set(Z, z);
                  });
                  let S = k.map((z) => L.get(z)).filter(Boolean),
                    H = S[A]?.index,
                    Q = T?.l?.id;
                  (A === S.length && S.length && (H = S[A - 1]?.index + 1),
                    H === void 0 &&
                      A > S.length &&
                      S.length &&
                      (H = S.at(-1)?.index + 1));
                  let at = dnd.F ? (dnd.ft?.types ?? []) : [],
                  ...
                  if (
                    ...
                    let z = new Set(q.map((Z) => Z.itemId));
                    if (z.size) {
                      let Z = k.reduce((Tt, U, et) => {
                        if (et >= A) return Tt;
                        let wt = L.get(U);
                        return Tt + (z.has(wt?.l?.id) ? 1 : 0);
                      }, 0);
                      Z && (H = Math.max(0, H - Z));
                    }
                  }
      ...
      this.fi(this.l.rating),
      (this.S = []),
      _(n) && (await this.sync({ L: this.L, h: t, Hi: !0, ka: e })),
      x && x.replaceWith(this.i),
  ...
  Nn() {
    this.o.u.Pe &&
      ...
      (this.o.u.Pe.Nn(),
      (this.m ??= []).push(
        J(this.o.u.Pe.o.i, "contextmenu", (t) => this.ya(t)),
      ));
  }
  af() {
    this.Ke ||
  ...
  setActive(t, e) {
    ...
    this.Mn && !yo(r, this.Mn) && r.scrollIntoView();
  }
  async df(t, e) {
  ...
  async Ge(t, e) {
    ...
    if (
      ...
      (ht(t, "label", (r) => this.mo(r)),
      ht(t, "title", (r) => this.mo(r)),
      ht(t, "archived", (r) => this.Bi(r)),
      ht(t, "color", (r) => this.setColor(r)),
      ht(t, "groupType", (r) => this.ze(r)),
      ht(t, "rating", (r) => this.fi(r)),
      ht(t, "locked", (r) => (this.l.locked = r)),
    ...
    if (
      (((this.Ot &&
        t.type === "update" &&
        Xt(["label"], Object.keys(t.propChanges)) &&
        t.updatedItemParentIds.includes(this.itemId)) ||
        (this.On &&
          t.type === "update" &&
          Xt(["archived"], Object.keys(t.propChanges)) &&
          t.updatedItemParentIds.includes(this.itemId)) ||
        (this.Ht &&
          ((jt(t.type, ["store", "move", "copy"]) &&
            t.affectedAncestorIds.includes(this.itemId)) ||
            (t.type === "update" &&
              Xt(["archived", "task"], Object.keys(t.propChanges)) &&
              t.affectedAncestorIds.includes(this.itemId)))) ||
    ...
    if (
      t.type === "store" &&
      jt(this.itemId, ["trash", "quickList", t.targetParentId])
    ) {
      await this.sync({ L: this.L });
    ...
    (this.ot &&
      this.Ht &&
      t.type === "update" &&
      Xt(["archived", "task", "done"], Object.keys(t.propChanges)) &&
      (await oneTabPage.ww.xw(n), await this.fu(n, t.updatedItemParentIds)),
      this.ot &&
        jt(t.type, ["store", "move", "copy", "removeFromQuickList"]) &&
  ...
  async sync({ L: t, h: e, Hi: s, ka: n }) {
    ...
    e ||
      ...
      this.Ht
        ? await e.tl()
        : this.Ss
          ? _(this.l) && (await e.Te({ groupId: this.itemId }))
          : await e.ke());
    ...
    let o = (r.childIds ?? [])
      ...
      .filter(({ id: a, index: l }) => {
        ...
        return !(
          !u ||
          (!this.Ss && !_(u)) ||
          (this.Ot && _(u) && !u.label && this.L.$a !== u.id) ||
          (this.B && !P(u)) ||
          (this.On && Zt(u)) ||
          (this.Ht && !(He(u) || e.Yu(u)))
        );
      });
    (It(this.o.u.Ia?.i, (a) => (a.style.display = o.length ? "none" : "block")),
      this.af(),
      !this.j &&
        (await Qs({
          Hi: s,
  ...
  ya(t) {
    (t.preventDefault(),
      ...
      (this.Tn = new dt({
        ...
        la: c({
          style: { margin: "8px" },
          children: Hs({ view: this, Vc: this.G, Ni: () => this.Tn.destroy() }),
        }),
      })));
  }
  Rc() {
...
async function Ae({ parentId: i, we: t, N: e, Ri: s }) {
  ...
  return (
    s && (s.L.$a = n),
    await v.Sa({ id: n, parentId: i, we: t, N: e }),
    n
  );
}
function eo(i) {
  ...
  return [
    ...new Set([
      ...
      ...$(e, "trash"),
      ...$(s, "untitled"),
      ...$(t, "isTreeItemInQuickListRoot"),
    ]),
  ];
}
function Hs({ Vc: i, view: t, Ni: e }) {
  return [
    dt.bi({
      label: h("selectAll"),
      name: "all",
...
class io {
  ...
  destroy() {
    (ao(this.Oa), (this.Oa = null), this.L?.De({}));
  }
}
class js {
  constructor({
...
const We = (i) => i.url && i.type !== "folder",
  Si = (i) => !i.url && i.type !== "bookmark";
function Us(i, t = !1, e, s, n) {
  let r = i.children.filter(We),
    o = i.children.some(Si);
  return c({
      ...
      body: c({
        ...
        children: {
          yw: c({
            ...
            children: {
              ...
              title: c({
                ...
                textContent: t ? h("allBookmarks") : i.title,
              }),
              gw: c({
                style: { flex: "0 0 auto" },
                ...(o && {
                  p: new Ce({
                    V: !0,
                    label: h("importFolderAndSubfolders"),
                    fontSize: "12px",
                    $: { marginInlineStart: "6px" },
                    Cs: { padding: "1px 5px" },
                    onclick: async (a) => {
                      ...
                      d.forEach((x) => (x.parentIds = []));
                      let p = [],
                        b = (x, m) => {
                          let g = x.children.some(Si),
                            w = x.children.some(We),
                            T = new Date().getTime(),
                            f;
                          g &&
                            ((f = {
                              id: Ht(),
                              label: x.title || h("importedBookmarks"),
                              type: "group",
                              groupType: "folder",
                              createDate: x.dateAdded ?? T,
                              modifyDate: x.dateGroupModified ?? T,
                              parentIds: [m.id],
                              childIds: [],
                              pinnedCount: 0,
                            }),
                            m.childIds.push(f.id),
                            p.push(f));
                          let k = 1e3,
                            A,
                            L = () => {
                          ...
                          (w && (A = L()),
                            x.children.forEach((S) => {
                              if (We(S)) {
                                let H = At(S.url),
                                  Q = d.find((at) => at.url === H);
                                (A.childIds.length >= k && (A = L()),
                                  A.childIds.push(Q.id),
                                  Q.parentIds.push(A.id));
                              } else b(S, f);
                            }));
                        },
                        y = await lt(s);
                      for (; !P(y); ) {
                        let x = y.parentIds.find(ot);
                        if (!x)
                          throw new Error("non-quicklist parent id not found");
                        y = await lt(x);
                      }
                      (b(i, y),
                        await v.In({ Lo: p, vn: d, O: y.id, An: [p[0].id] }),
                        n());
                    },
                  }),
                }),
          ...
          ...(r.length && {
            kw: new js({
              wf: c({
                ...
                children: {
                  label: c({ style: {}, textContent: hr(r.length) }),
                  bf: c({
                    style: { flex: "0 0 auto" },
                    p: new Ce({
                      V: !0,
          ...
          Me: c({
            ...
            children: {
              ...i.children.filter(Si).map((a) => Us(a, !1, e, s, n)),
              ...(!i.children.length && {
                Iw: c({
                  style: { paddingInlineStart: "26px", fontSize: "12px" },
                  textContent: h("emptyInBrackets"),
                }),
              }),
            },
          }),
...
function qs(i) {
  ...
  return c({
    className: [...$(i.hasBeenRead, "archived")].join(" "),
    style: {
...
function Ml(i) {
  return c({ style: { fontSize: "1px", height: i + "px", width: "1px" } }).i;
}
let Ns = !1;
...
const v = fi(
  new Proxy(
    ...
    {
      get(i, t, e) {
        return (...s) => ((s = di(s, void 0, sr)), ro(t, { args: s }));
      },
    },
  ),
  "core",
...
let Rs = Qe.getContext("2d"),
  Ws = "data:image/png;base64,",
  co = Ws.length;
function zs(i) {
  (Rs.clearRect(0, 0, 32, 32), Rs.drawImage(i, 0, 0, 32, 32));
  let t = Qe.toDataURL("image/png");
  return t.startsWith(Ws) ? t.substring(co) : t;
}
async function Zs(i) {
  let { Sw: t, Ow: e } = await v.Mw(i);
  return t ? (await fo(), []) : e;
}
function Li() {
  return h("chromeIncognitoInstructions");
}
async function fo() {
  let i = h("incogitoPermissionRequired") + " " + Li(),
    t = h("privateBrowsingPermissionRequired") + " " + Li();
  await te(i);
}
function Pi({ style: i = {}, uh: t = 7 }) {
  let e = c({
    className: "spinner",
    style: { opacity: "1", ...i },
    children: Object.fromEntries(
      Vn(t).map((s) => [s, document.createElement("div")]),
    ),
  }).i;
...
async function Gi({
  ...
  (r || (r = h("locationColon")), await i.ke());
  let o = await i.Hn(t, s);
  return M("span", {
    style: { fontSize: e ?? "12px" },
    children: {
      ...(n && { Gw: M("span", { textContent: `${r}  ` }) }),
      ...pi(
        o.map((a, l) => ({
          [`loc${l}`]: M("span", {
            dir: "auto",
            style: { cursor: "pointer" },
            textContent: ye({ groupId: a.id, h: i, l: a }),
            onclick: async (u) => {
              await v.ec({ itemId: a.id, Zi: l === 0 ? a.id : o[l - 1].id });
            },
          }),
        })),
        () => M("span", { textContent: `  ${K() ? "➝" : "⭠"}  ` }),
      ),
    },
  });
}
async function Js({ Et: i, groups: t, h: e, he: s, oe: n }) {
  return (
          ...
          p: c({
            ...
            children: [
              ...
              new Vr({
                ...
                marginBlock: "0px 0px",
                label: a.label || (V(a) ? h("all") : h("untitled")),
                oe: n,
                qd: () => s(a),
              }),
            ],
  ...
  try {
    ...
    let e = c({
      ...
      children: {
        Dw: M("div", {
          children: {
            a: M("span", { textContent: h("oneTabRestartRequired") + " " }),
            b: M("a", {
              style: { cursor: "pointer", textDecoration: "underline" },
              onclick: () => chrome.runtime.reload(),
              textContent: h("clickHereToContinue"),
            }),
          },
        }),
        Pw: M("div", {
          p: M("div", {
            ...
            textContent: h("mayNeedToRestartBrowser"),
          }).i,
        }),
      },
    });
...
async function Qs({
  ...
  for (let x = 0; x < o.length; x++) {
    let m = o[x],
      g = e.findIndex((f, k) => k >= x && s(f) === m.id),
      w = e[g],
      T = w && n(w);
    if (!i && T && !w?.tu)
      (g > x && (e.splice(g, 1), e.splice(x, 0, w)),
        t.children.item(x) !== T && t.insertBefore(T, t.children.item(x)),
      ...
      let f = g >= x ? e[g] : void 0;
      if ((b % d === 0 && (await Ut(0)), (w = await r(m.id, m.index)), w)) {
        let k = n(w);
        if (!k) (w.De?.({ pi: !0 }), (w = f));
        else {
          let A = x;
          if (f) {
            let L = e.indexOf(f);
            L >= 0 && (f.De?.({ pi: !0 }), e.splice(L, 1), L < A && A--);
          }
          (t.insertBefore(k, t.children.item(A)), e.splice(A, 0, w));
        }
      } else f && (w = f);
    }
    w && ((w.index = m.index), u?.(w));
  }
  if ((a?.() && (p = !0), p)) {
    l();
    return;
  ...
  for (; e.length > o.length; ) {
    ...
    (x.De({ pi: g }), e.splice(e.length - 1, 1));
  }
  let y = new Set(e.map((x) => n(x)).filter(Boolean));
...
let Ie;
async function xo() {
  ((Ie = await v.ks("theme")), Di());
}
function wo() {
  return !window.matchMedia?.("(prefers-color-scheme: dark)").matches;
}
async function Dl(i) {
  (await v.bt("theme", i), (Ie = i), Di());
}
let Ct;
function Di() {
  ((Ct = { auto: wo(), light: !0, dark: !1 }[Ie]),
    mo(),
    [...document.querySelectorAll(".lightDarkInnerImg")].forEach((i) => {
      i.src = i.dataset[Ct ? "lightSrc" : "darkSrc"];
    }),
    Ks.forEach((i) => i()));
}
function mo() {
...
function bo() {
  return c({
    ...
    children: [
      ...
      c({ textContent: h("returnToOneTab") }),
    ],
  });
}
function yo(i, t) {
...
async function Xs(i) {
  let t = new kt(),
    e = await t.ht(i);
  return (P(e) || (await t.Te({ groupId: i })), t);
}
function he(i) {
...
async function Ke({ rt: i, O: t, N: e }) {
    ...
    );
  {
    let o = new kt();
    await o.ke();
    let [a, l] = li(i, (y) => y.sourceParentId),
      u = new Set(
    ...
    for (let y of d) {
      ...
      if (
        (g.forEach((T) =>
          T.childIds
            .filter((f) => f === m.id)
            .forEach((f, k) => w.push(gt({ itemId: m.id, Fe: T.id, zt: k }))),
        ),
        t === "trash")
      )
        w.forEach((T) => a.push(T));
      else {
        let T = w[0];
        (a.push(T), w.slice(1).forEach((k) => b.push(k)));
      }
    }
    if (b.length) {
      let y = await v.gs(b);
  ...
  if (t === "trash") {
    let o = await v.gs(i);
    (await v.move({ rt: i, O: t, N: e }), await v.Oe(o));
  } else if (s.some(j) && P(n)) {
    let o = await tn({ O: t, N: e }),
      a = await v.gs(i);
    (await v.move({ rt: i, O: o }), await v.Oe(a));
  } else if (s.some(vt) && vt(n)) {
    let o = 0;
    for (let a of i) {
      let l = a.itemId,
        u = await lt(l);
      if (vt(u)) {
        let d = await ge(u.childIds);
        if (!0) {
          let b = [];
          (d.forEach((y, x) => {
            j(y)
              ? b.push(
                  gt({
                    itemId: y.id,
                    zt: u.childIds.slice(0, x).filter((m) => m === y.id).length,
                    Fe: u.id,
                  }),
                )
              : b.push(gt({ itemId: y.id }));
          }),
            await v.move({ rt: b, O: t, N: e === void 0 ? void 0 : e + o }),
            (o += d.length),
            await v.Oe([a.itemId], !0));
        } else {
          let [b, y] = li(d, j);
          (y.length &&
            (await v.move({
              rt: y.map((x) => gt({ itemId: x.id })),
              O: t,
              N: e === void 0 ? void 0 : e + o,
            }),
            (o += y.length)),
            b.length
              ? (await v.Fi(u.id, { groupType: "tabGroup", color: me }),
                await v.move({
                  rt: [a],
                  O: t,
        ...
        let d = await v.gs([a]);
        (await v.move({ rt: [a], O: t, N: e === void 0 ? void 0 : e + o++ }),
          await v.Oe(d));
      }
    }
  } else if ((s.some(ct) || s.some(vt)) && ct(n)) {
    let o = new kt(),
      a = [],
      l = [];
    const u = (d, p, b) => {
      if (_(d)) {
        let y = d;
        (o.ul(y.childIds).forEach((m, g) => u(m, d, g)), l.push(y));
      } else
        a.push(
          gt({
            itemId: d.id,
            zt: p.childIds.slice(0, b).filter((y) => y === d.id).length,
            Fe: p.id,
          }),
        );
    };
    for (let d of i) {
      let p = d.itemId;
      await o.Te({ groupId: p });
      let b = o.v(p),
        y = await o.ht(b.parentIds.find(ot));
      u(b, y);
    }
    (a.length && (await v.move({ rt: a, O: t, N: e })),
      await v.Oe(l.map(Vt), !0));
  } else {
    let o = await v.gs(i);
    (await v.move({ rt: i, O: t, N: e }), await v.Oe(o));
  }
...
async function To(i) {
  return await Promise.all(
    i.map(async (t) => {
      ...
      return { fl: t, Eo: await ni(e), kt: e, At: t.id, di: t.groupId };
    }),
  );
}
function Ao(i) {
...
const Ul = Symbol("dropHandled"),
  ql = Symbol("dragOverHandled"),
  rt = c({
    id: "landingDiv",
    style: {
