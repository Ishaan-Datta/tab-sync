...
`,
    ...
    function filterNested(items, predicate) {
      let result = items.map((item) => (Array.isArray(item) ? filterNested(item, predicate) : item));
      return (result = result.filter((item) => !Array.isArray(item) || item.length > 0)), result.filter((item) => Array.isArray(item) || predicate(item));
    }
    ...
    function partition(items, predicate) {
      return items.reduce(
        ([left, right], item, index) => ((predicate(item, index) ? left : right).push(item), [left, right]),
        [[], []],
      );
    }
    ...
    async function asyncPartition(items, predicate) {
      let values = await Promise.all(items.map((item, index) => predicate(item, index)));
      return items.reduce(([left, right], item, index) => ((values[index] ? left : right).push(item), [left, right]), [[], []]);
    }
    ...
    function isExcludedUrl(url) {
      if (!url || url === "") return !0;
      for (let prefix of excludedUrlPrefixes) if (url.startsWith(prefix) && !isPlaceholderUrl(url)) return !0;
    ...
    async function getUncommittedChanges(key) {
      return ((await getCoreProxy()._e("uncommittedChangesStore")) ?? {})?.[key]?.uncommittedChanges ?? {};
    }
    ...
    async function pruneUncommittedChanges(scheduleNext) {
      ...
      ((store = Object.fromEntries(Object.entries(store).filter(([, entry]) => entry.modifyDate > cutoff))),
        await getCoreProxy().Ve("uncommittedChangesStore", store),
        scheduleNext && setTimeout(() => pruneUncommittedChanges(), 1e3 * 3600 * 24 * 7));
    }
